...
const buildHtml = ({ type, payload, context }) => {
  ...
  const contextRows = [
    context?.planLabel || context?.plan ? `<li><strong>Plan:</strong> ${context?.planLabel || context?.plan}</li>` : '',
    context?.billingLabel || context?.billing ? `<li><strong>Billing:</strong> ${context?.billingLabel || context?.billing}</li>` : '',
    context?.payrollInterest ? `<li><strong>Payroll:</strong> ${context?.payrollLabel || 'Interested'}</li>` : '',
    context?.promoLabel || context?.promo ? `<li><strong>Offer:</strong> ${context?.promoLabel || context?.promo}</li>` : '',
...
export default async function handler(request, response) {
  ...
  const textBody = String(body.body || serializeLeadBody({ type: body.type, payload, context: body.context }))
