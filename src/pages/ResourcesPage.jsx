...
export default function ResourcesPage() {
  ...
  return (
    <>
      <SeoManager meta={content?.seo?.resources} schema={[organizationSchema, websiteSchema, softwareSchema]} />
      <PageHero title={page.heroTitle} subtitle={page.heroSubtitle} preview={{ type: 'expenses' }} />
        ...
        <div className="grid gap-6 lg:grid-cols-2 xl:grid-cols-4">
          {(page.sections || []).map((section) => (
            <Card key={section.title}>
              ...
              <Link to={section.path || '/product'} className="zx-button zx-button-secondary mt-5 w-full justify-center">
                {section.cta}
              </Link>
            </Card>
          ))}
            ...
            <Card className="h-full">
              <div className="space-y-5">
                {page.demoEnvironment?.accessTitle ? (
                  <div>
                    <p className="text-xs uppercase tracking-[0.14em] text-zx-accent">{page.demoEnvironment.accessTitle}</p>
                    {page.demoEnvironment?.accessIntro ? <p className="mt-2 text-sm text-zx-text-muted">{page.demoEnvironment.accessIntro}</p> : null}
                  </div>
                ) : null}
                {showLegacySharedCredentials ? (
                  <>
                    <div>
                      ...
                      <p className="mt-2 text-base font-semibold text-zx-text">{page.demoEnvironment?.emailValue}</p>
                    </div>
                    <div>
                      <p className="text-xs uppercase tracking-[0.14em] text-zx-text-muted">{page.demoEnvironment?.passwordLabel}</p>
                      <p className="mt-2 text-base font-semibold text-zx-text">{page.demoEnvironment?.passwordValue}</p>
                    </div>
                  </>
                ) : null}
              </div>
        ...
        <Card className="mt-8">
          <ul className="grid gap-2 text-sm text-zx-text-muted sm:grid-cols-2">
            {(page.sitemap?.items || []).map((item) => (
              <li key={item} className="rounded-lg border border-zx-border bg-zx-surface-strong px-3 py-2">
                {item}
              </li>
            ))}
          </ul>
      ...
      <section className="section-shell pb-16">
        ...
        <div className="mt-8 grid gap-6 lg:grid-cols-3">
          <Card>
            <h3 className="font-heading text-lg font-semibold text-zx-text">{page.design?.directionTitle}</h3>
          ...
          <Card>
            <h3 className="font-heading text-lg font-semibold text-zx-text">{page.design?.componentsTitle}</h3>
            <ul className="mt-3 space-y-2 text-sm text-zx-text-muted">
          ...
          <Card>
            <h3 className="font-heading text-lg font-semibold text-zx-text">{page.design?.statesTitle}</h3>
            <ul className="mt-3 space-y-2 text-sm text-zx-text-muted">
