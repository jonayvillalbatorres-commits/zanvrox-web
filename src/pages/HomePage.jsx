...
export default function HomePage() {
  ...
  return (
    <>
      <SeoManager meta={content?.seo?.home} schema={[organizationSchema, websiteSchema, softwareSchema, homeFaqSchema]} />
      ...
      <PageHero
        ...
        secondaryCta={heroSecondaryIsExternal ? { ...page.heroSecondaryCta, path: heroSecondaryHref } : page.heroSecondaryCta}
        preview={{
      ...
      <section className="section-shell">
        <div className="zx-glass-strip grid gap-3 px-5 py-5 sm:grid-cols-2 lg:grid-cols-4">
          {(page.trustSignals || []).map((signal) => (
            <div key={signal} className="rounded-xl border border-zx-border bg-zx-surface-strong px-4 py-3">
      ...
      <section className="section-shell">
        <div className="zx-glass-strip grid gap-3 px-5 py-5 sm:grid-cols-2 lg:grid-cols-3">
          {(page.statusBadges || []).map((item) => (
            <div key={item.text} className="rounded-xl border border-zx-border bg-zx-surface-strong px-3 py-3">
      ...
      <section className="section-shell">
        <SectionHeading eyebrow={page.screenshots?.eyebrow} title={page.screenshots?.title} subtitle={page.screenshots?.subtitle} />
      ...
      <section className="section-shell">
        <SectionHeading eyebrow={page.howItWorks?.eyebrow} title={page.howItWorks?.title} subtitle={page.howItWorks?.subtitle} />
          ...
          <div className="mt-8 grid gap-4 md:grid-cols-[1.2fr_0.8fr]">
            <div className="grid gap-3">
              {(page.compliance?.bullets || []).map((bullet) => (
                <div key={bullet} className="rounded-xl border border-zx-border bg-zx-surface-strong px-4 py-3 text-sm text-zx-text-muted">
                  {bullet}
                </div>
              ))}
            </div>
            <div className="rounded-xl border border-zx-border bg-zx-surface-strong px-4 py-4">
              <p className="text-xs uppercase tracking-[0.14em] text-zx-accent">{page.compliance?.coverageLabel}</p>
      ...
      <section className="section-shell">
        <SectionHeading eyebrow={page.included?.eyebrow} title={page.included?.title} subtitle={page.included?.subtitle} />
      ...
      <section className="section-shell">
        <div className="grid gap-6 lg:grid-cols-[1.2fr_0.8fr]">
          <div className="zx-card">
            ...
            <h2 className="mt-4 font-heading text-2xl font-semibold text-zx-text">{page.executiveSnapshot?.title}</h2>
            ...
            <div className="mt-5 grid gap-3 sm:grid-cols-3">
              {(page.executiveSnapshot?.metrics || []).map((metric) => (
                <div key={metric.label} className="zx-metric-card">
                  ...
                  <p className="mt-1 text-[0.72rem] uppercase tracking-[0.16em] text-zx-accent">{metric.label}</p>
          ...
          <div className="zx-card">
            <SectionHeading eyebrow={page.technical?.eyebrow} title={page.technical?.title} subtitle={page.technical?.subtitle} />
      ...
      <section className="section-shell">
        <div className="zx-card" id="demo-environment">
          <SectionHeading title={page.demoEnvironment?.title} subtitle={page.demoEnvironment?.subtitle} />
          <div className="mt-8 grid gap-6 lg:grid-cols-[0.9fr_1.1fr]">
            <Card className="h-full">
              <div className="grid gap-4">
                {showLegacySharedCredentials ? (
                  <>
                    <div>
                      <p className="text-xs uppercase tracking-[0.14em] text-zx-text-muted">{page.demoEnvironment?.emailLabel}</p>
                      <p className="mt-2 text-lg font-semibold text-zx-text">{page.demoEnvironment?.emailValue}</p>
                    </div>
                    <div>
                      <p className="text-xs uppercase tracking-[0.14em] text-zx-text-muted">{page.demoEnvironment?.passwordLabel}</p>
                      <p className="mt-2 text-lg font-semibold text-zx-text">{page.demoEnvironment?.passwordValue}</p>
                    </div>
                  </>
                ) : null}
            ...
            <div className="grid gap-3">
              {(page.evaluationPath?.cards || []).slice(0, 3).map((card) => (
                <div key={`${page.demoEnvironment?.title}-${card.title}`} className="rounded-xl border border-zx-border bg-zx-surface-strong px-4 py-3 text-sm text-zx-text-muted">
                  {card.body}
                </div>
              ))}
      ...
      <section className="section-shell">
        <SectionHeading eyebrow={page.positioning?.eyebrow} title={page.positioning?.title} subtitle={page.positioning?.subtitle} />
        <div className="mt-10 grid gap-6 md:grid-cols-2">
          {(page.positioning?.cards || []).map((item) => (
            <Card key={item.role}>
              <p className="text-xs font-semibold uppercase tracking-[0.12em] text-zx-accent">{item.role}</p>
              <h3 className="mt-2 font-heading text-xl font-semibold text-zx-text">{item.headline}</h3>
      ...
      <section className="section-shell pb-16">
        <div className="zx-cta-panel rounded-3xl border border-zx-border p-8 shadow-panel">
          ...
          <div className="mt-6 flex flex-wrap gap-3">
            ...
            <Link to={page.cta?.secondary?.path || '/pricing'} className="zx-button zx-button-secondary">
