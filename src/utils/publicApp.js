...
const resolveRuntimePublicAppUrl = () => {
  const configured = normalizeBaseUrl(import.meta.env.VITE_PUBLIC_APP_URL || import.meta.env.VITE_APP_URL || '')
...
export const isGuidedDemoLoginUrl = (path) => /\/login(?:\?|$)/i.test(String(path || '')) && /[?&]demo=1(?:&|$)/i.test(String(path || ''))
