      ...
      <div className="hero-grid">
        <div className="grid gap-8 lg:grid-cols-[1.1fr_0.9fr] lg:items-center">
          <div className="space-y-6">
            {kicker ? <p className="text-xs font-semibold uppercase tracking-[0.16em] text-zx-accent">{kicker}</p> : null}
            <h1 className="font-heading text-4xl font-semibold text-zx-text sm:text-5xl">{title}</h1>
          ...
          <aside className="reveal-up">
            <div className="relative overflow-hidden rounded-2xl border border-zx-border bg-zx-surface-strong p-4 shadow-panel backdrop-blur-lg">
              ...
              <div className="relative z-10">
                <p className="text-xs font-semibold uppercase tracking-[0.16em] text-zx-accent">{preview?.title || 'Product Preview'}</p>
                <div className="mt-3">
                  <ProductPreview type={preview?.type} imageSrc={preview?.imageSrc} imageAlt={preview?.imageAlt || 'Product preview'} />
                </div>
                <p className="mt-3 text-xs uppercase tracking-[0.14em] text-zx-text-muted">{preview?.caption || 'Live inside the ERP'}</p>
