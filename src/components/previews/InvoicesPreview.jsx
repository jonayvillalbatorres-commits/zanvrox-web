        ...
        <div className="mt-3 overflow-hidden rounded-md border border-zx-border bg-zx-bg/60">
          ...
          <div className="divide-y divide-zx-border/70">
            {invoices.map((item) => (
              <div
                key={item.id}
                className="grid grid-cols-[1fr_1.4fr_0.8fr_0.7fr] items-center px-3 py-2 text-xs"
              >
                ...
                <span
                  className={`inline-flex w-fit rounded-full border px-2 py-0.5 text-[10px] ${statusClass[item.tone]}`}
                >
                  {t(item.status)}
                </span>
              </div>
