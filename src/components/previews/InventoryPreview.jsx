    ...
    <div className="aspect-[16/10] bg-gradient-to-br from-[#091426] via-[#0c1d37] to-[#0b1323] p-3 sm:p-4">
      <div className="h-full rounded-lg border border-zx-border bg-zx-surface/70 p-3">
        <div className="flex items-center justify-between">
          ...
          <span className="rounded-full border border-zx-warning/40 bg-zx-warning/10 px-2 py-0.5 text-[10px] text-zx-warning">
            {t('2 low-stock alerts')}
          </span>
        </div>
        ...
        <div className="mt-3 space-y-2">
          {items.map((item) => {
            const ratio = Math.max(
              6,
              Math.min(100, Math.round((item.stock / (item.min * 2)) * 100))
            );
            return (
              <div
                key={item.sku}
              ...
              >
                ...
                <div className="mt-1.5 h-1.5 w-full overflow-hidden rounded-full bg-zx-surface-strong">
                  <div
                    className={`h-full rounded-full ${item.tone === 'low' ? 'bg-zx-warning' : 'bg-zx-success'}`}
                    style={{ width: `${ratio}%` }}
                  />
                </div>
              </div>
