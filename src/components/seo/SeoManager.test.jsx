...
describe('SeoManager', () => {
  test('writes canonical, meta tags, and full hreflang set', async () => {
    ...
    expect(document.head.querySelector('meta[property="og:title"]')).toHaveAttribute('content', meta.title)
    expect(document.head.querySelector('link[rel="canonical"]')).toHaveAttribute(
      'href',
      `https://zanvrox.com${meta.path}`,
    )

    const alternateLinks = Array.from(document.head.querySelectorAll('link[data-zx-hreflang="true"]'))
