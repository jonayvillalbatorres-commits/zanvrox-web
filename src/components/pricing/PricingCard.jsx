...
export default function PricingCard({ tier, pricing, language = 'en', billingPeriod = BILLING_PERIODS.monthly }) {
  ...
  const includedItems = (tier?.included || []).map((item) => String(item || '').trim()).filter(Boolean)
  ...
  return (
    ...
    >
      <div className="absolute -top-3 left-5 flex flex-wrap gap-2">
        {isPopular ? <span className="zx-badge border-zx-accent bg-zx-accent/10 text-zx-accent">{pricing?.popularLabel}</span> : null}
        {activePricing?.badge ? <span className="zx-badge zx-warning-soft text-zx-warning">{activePricing.badge}</span> : null}
      </div>
      ...
      <div className="mt-6 rounded-2xl border border-zx-border bg-zx-surface-strong/80 p-4">
        ...
        <p className="mt-2 text-4xl font-semibold text-zx-text">
          ...
          {activePricing?.kind === 'custom' ? null : <span className="ml-2 text-base font-medium text-zx-text-muted">{priceSuffix}</span>}
        </p>
        {activePricing?.note ? <p className="mt-2 text-sm text-zx-text-muted">{activePricing.note}</p> : null}
        {activePricing?.secondaryNote ? <p className="mt-3 text-xs text-zx-accent">{activePricing.secondaryNote}</p> : null}
      </div>
      ...
      <div className="mt-5 grid gap-3 text-sm text-zx-text-muted sm:grid-cols-2">
        <div className="rounded-xl border border-zx-border bg-zx-surface-strong/70 px-3 py-3">
          <p className="text-xs uppercase tracking-[0.14em] text-zx-text-muted">{pricing?.entitiesIncludedLabel}</p>
          <p className="mt-2 font-medium text-zx-text">{tier?.entitiesLabel}</p>
        </div>
        <div className="rounded-xl border border-zx-border bg-zx-surface-strong/70 px-3 py-3">
          <p className="text-xs uppercase tracking-[0.14em] text-zx-text-muted">{pricing?.usersIncludedLabel}</p>
      ...
      <div className="mt-5 space-y-3 text-sm text-zx-text-muted">
        <div>
          <p className="text-xs uppercase tracking-[0.14em] text-zx-text-muted">{pricing?.includesLabel}</p>
