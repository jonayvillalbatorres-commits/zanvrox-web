...
import { createLeadInitialState, LEAD_TYPES, validateLeadPayload } from '../../utils/contactLeadValidation'

export default function DemoForm({ formContent, leadContext = null }) {
  ...
  const onSubmit = async (event) => {
    ...
    try {
      ...
      setSubmitError(formContent?.submitError || 'We could not submit your request. Please try again.')
    } finally {
      setSubmitting(false)
    }
  }
  ...
  return (
    <section className="zx-card" id="demo-form">
      ...
      {submitted ? <p className="zx-success-soft mt-4 rounded-lg border px-4 py-3 text-sm text-zx-success">{formContent?.confirmation}</p> : null}
      {submitError ? <p className="mt-4 rounded-lg border border-zx-danger/40 bg-zx-danger/10 px-4 py-3 text-sm text-zx-danger">{submitError}</p> : null}

      <form className="mt-6 space-y-4" noValidate onSubmit={onSubmit}>
        <input type="text" name="website" value={form.website} onChange={setField} className="hidden" tabIndex={-1} autoComplete="off" />
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <label className="space-y-2 text-sm text-zx-text-muted">
            ...
            />
            {errors.company ? <span className="text-xs text-zx-danger">{errors.company}</span> : null}
          </label>
        </div>
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <label className="space-y-2 text-sm text-zx-text-muted">
            ...
            {errors.companySize ? <span className="text-xs text-zx-danger">{errors.companySize}</span> : null}
          </label>
        </div>
        ...
        <label className="space-y-2 text-sm text-zx-text-muted">
          ...
          {errors.interestArea ? <span className="text-xs text-zx-danger">{errors.interestArea}</span> : null}
        </label>
        ...
        <label className="space-y-2 text-sm text-zx-text-muted">
          ...
          {!errors.message && formContent?.messageHint ? <span className="text-xs text-zx-text-muted">{formContent.messageHint}</span> : null}
        </label>
        ...
        {hasValidated && liveErrors.startedAt ? <p className="text-xs text-zx-danger">{liveErrors.startedAt}</p> : null}

        <button type="submit" className="zx-button zx-button-primary w-full sm:w-auto" disabled={submitting}>
